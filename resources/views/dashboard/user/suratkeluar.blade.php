...
<div class="d-flex justify-content-between align-items-center">
    ...
    <form class="search w-50" action="{{ route('suratkeluar') }}" method="GET">
        ...
        <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <tbody>
                            ...
                            <th scope="row">{{ $loop->iteration }}</th>
                                ...
                                <td class="text-center">
                                    ...
                                    <a href="/surat/show/{{ $item->id }}" class="btn btn-primary btn-sm"><i
                                            class="bi bi-eye-fill"></i></a>
                                    <a href="/surat/edit/{{ $item->id }}" class="btn btn-success btn-sm"><i
                                            class="bi bi-eyedropper"></i></a>
                                    <a href="/surat/delete/{{ $item->id }}" class="btn btn-danger btn-sm"
                                        onclick="return confirm('Apakah Anda Yakin Untuk Hapus Surat {{ $item->pengirim }}?')"><i
            ...
            <div class="d-flex justify-content-between align-items-center mt-3">
                ...
                <div class="column-info">
                    <span class="badge ">Menampilkan {{ count($suratKeluar) }} dari {{ $suratKeluar->total() }}
                        surat</span>
                </div>
                ...
                <ul class="pagination mb-0">
                    <!-- Tombol Previous -->
                    <li class="page-item {{ $suratKeluar->onFirstPage() ? 'active' : '' }}">
                        <a class="page-link" href="{{ $suratKeluar->previousPageUrl() }}">Previous</a>
                    </li>

                    <!-- Nomor Halaman -->
                    @for ($i = 1; $i <= $suratKeluar->lastPage(); $i++)
                        <li class="page-item {{ $suratKeluar->currentPage() == $i ? 'active' : '' }}">
                            <a class="page-link" href="{{ $suratKeluar->url($i) }}">{{ $i }}</a>
                        </li>
                    @endfor

                    <!-- Tombol Next -->
                    <li class="page-item {{ $suratKeluar->hasMorePages() ? '' : 'active' }}">
                        <a class="page-link" href="{{ $suratKeluar->nextPageUrl() }}">Next</a>
                    </li>
                </ul>
            </div>
        </div>
